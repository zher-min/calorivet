...
test('home-screen manifest launches VetTools standalone at the dashboard', async () => {
  const manifest = JSON.parse(await readFile(new URL('../public/manifest.webmanifest', import.meta.url), 'utf8'));
  assert.equal(manifest.id, '/');
  assert.equal(manifest.start_url, '/');
  assert.equal(manifest.scope, '/');
  assert.equal(manifest.short_name, 'VetTools');
  ...
  const apple = await readFile(new URL('../public/icons/vettools-apple-touch-icon.png', import.meta.url));
...
test('both calculator pages expose manifest and Apple installation metadata', async () => {
  ...
  for (const route of ['/', '/calculators/calorie', '/calculators/transfusion']) {
    ...
    assert.match(html,/\/icons\/vettools-apple-touch-icon.png/);
