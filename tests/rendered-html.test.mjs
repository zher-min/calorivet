...
test("dashboard and transfusion direct routes render inside VetCalc", async () => {
  ...
  for (const title of ["Blood Transfusion", "Blood product PCV/HCT", "Donor calculator", "Clinical notes", "References"]) assert.ok(page.includes(title), title);
  assert.doesNotMatch(page, /not yet available/);
  assert.match(page, /Enter weight and valid PCV\/HCT values/);
  assert.doesNotMatch(page, />Calculate<|Administration &amp; monitoring|Previous transfusion\?|Planned duration|Empirical feline whole-blood estimate/);
  assert.doesNotMatch(page, /<details[^>]*\sopen/);
  assert.match(page, /aria-pressed="true">20(?:<!-- -->)?%/);
  assert.match(page, /class="species-emoji"/);
  assert.match(page, /🐕/);
  assert.match(page, /🐈/);
  assert.doesNotMatch(page, /<select[^>]*aria-label="Species"/);
  assert.doesNotMatch(page, /aria-label="Current transfusion estimate"/);
